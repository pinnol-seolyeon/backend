...
public record OrdererResDto(
	...
	public static OrdererResDto toDto(OrdererProfile profile) {
		return new OrdererResDto(
			profile.getId(),
			profile.getUserId(),
			profile.getOrdererName(),
			profile.getOrdererPhone(),
			profile.getOrdererEmail()
		);
	}
}
